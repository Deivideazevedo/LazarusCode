...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["lazaruscode.json", "lazaruscode.jsonc", "config.json", "opencode.json", "opencode.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".lazaruscode", ".opencode", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".lazaruscode" || path.basename(item) === ".opencode")
        .toReversed()
