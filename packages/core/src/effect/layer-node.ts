...
function walk<Result>(
  ...
  const recur = (node: AnyNode): Result => {
    ...
    const target = node?.name ? options.resolve?.(node) ?? node : node
  ...
  const node = walk<AnyNode>(
    ...
    (node, context) => {
      ...
      if (node.kind === "group") {
        return {
          ...node,
          dependencies: (node.dependencies ?? []).filter(Boolean).map(context.visit).filter(Boolean),
        }
      }
      if (node.tag === tag) {
        if (!node.name) return group([])
    ...
    { resolve: (node) => (node?.name ? replacementMap.get(node.name) ?? node : node) },
  )

  return {
    node: (node ?? group([])) as Node<A, E>,
    hoisted: group(Array.from(hoisted.values()).filter(Boolean)) as Node<unknown, E>,
  }
}
...
export function compile<A, E, const Items extends Replacements = readonly []>(
  ...
  const compileNode = (node: AnyNode) =>
    walk<RuntimeLayer>(
      node,
      (node, context) => {
        if (!node) return Layer.empty as RuntimeLayer
      ...
      { cache, resolve: (node) => (node?.name ? replacementMap.get(node.name) ?? node : node) },
    )
...
function replacementMapFrom(replacements?: Replacements) {
  return (
    replacements
      ?.filter((entry): entry is Replacement => Boolean(entry && entry[0] && entry[1]))
      .reduce((map, [source, replacement]) => {
        if (!source?.name || !replacement) return map
...
function rewriteReplacementDependencies(root: AnyNode, replacements: ReadonlyMap<string, AnyNode>) {
  if (!root || replacements.size === 0) return root
  ...
  const recur = (node: AnyNode, isRoot = false): AnyNode => {
    if (!node) return undefined as any
    const target = isRoot ? node : (node?.name ? replacements.get(node.name) ?? node : node)
    if (!target) return undefined as any
    const cached = cache.get(target)
    if (cached !== undefined || cache.has(target)) return cached!
    ...
    try {
      const dependencies = (target.dependencies ?? [])
        .filter(Boolean)
        .map((dependency) => recur(dependency))
        .filter(Boolean)
...
export function hasUnbound(root: Node<unknown, unknown, any>, source: AnyNode): boolean {
  ...
  return walk<boolean>(root, (node, context) => {
    ...
    return (node.dependencies ?? []).filter(Boolean).some(context.visit)
  })
}

function flatten(node: AnyNode): readonly AnyNode[] {
  if (!node) return []
  return node.kind === "group" ? (node.dependencies ?? []).filter(Boolean).flatMap(flatten) : [node]
}
