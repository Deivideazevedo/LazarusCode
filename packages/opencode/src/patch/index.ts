...
import { Codepage } from "../util/codepage"
...
export const applyHunksToFiles = Effect.fn("Patch.applyHunksToFiles")(function* (hunks: Hunk[]) {
  ...
  for (const hunk of hunks) {
    switch (hunk.type) {
      case "add": {
        yield* fs.writeWithDirs(hunk.path, Codepage.prepareWrite(hunk.contents, hunk.path, false))
      ...
      case "update": {
        const originalText = Codepage.isPowerBuilderFile(hunk.path)
          ? Codepage.decode(yield* fs.readFile(hunk.path), hunk.path)
        ...
        if (hunk.move_path) {
          yield* fs.writeWithDirs(
            hunk.move_path,
            Codepage.prepareWrite(fileUpdate.content, hunk.move_path, fileUpdate.bom),
          )
          yield* fs.remove(hunk.path)
          modified.push(hunk.move_path)
          yield* Effect.logInfo(`Moved file: ${hunk.path} -> ${hunk.move_path}`)
        } else {
          yield* fs.writeWithDirs(hunk.path, Codepage.prepareWrite(fileUpdate.content, hunk.path, fileUpdate.bom))
      ...
      for (const hunk of args.hunks) {
        ...
        switch (hunk.type) {
          ...
          case "update": {
            ...
            const originalText = yield* (
              Codepage.isPowerBuilderFile(updatePath)
                ? fs.readFile(updatePath).pipe(Effect.map((buf) => Codepage.decode(buf, updatePath)))
                : fs.readFileString(updatePath)
            ).pipe(
              Effect.catch((cause) =>
