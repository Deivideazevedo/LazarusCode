...
import { z } from "zod"
import { createLogger, logger as _logger, type Logger as _Logger, type LogLevel as _LogLevel } from "./logger.js"
import {
...
export const PLUGIN_VERSION: string = ((globalThis as Record<string, unknown>).__PLUGIN_VERSION__ as string) ?? "dev"
...
> & {
  ...
  const providerId = rawProviderId.startsWith("opencode-") ? rawProviderId : `opencode-${rawProviderId}`
  const displayName =
    opts?.displayName ??
    (providerId === `opencode-${OMNIROUTE_PROVIDER_KEY}` ? "OmniRoute" : `OmniRoute (${providerId})`)
  const modelCacheTtl =
    typeof opts?.modelCacheTtl === "number" && opts.modelCacheTtl > 0 ? opts.modelCacheTtl : DEFAULT_MODEL_CACHE_TTL_MS
 ...
 */
export function invalidateOmniRouteFetchCache(cache: OmniRouteFetchCache, baseURL?: string): number {
  if (!baseURL) {
    const n = cache.size
    cache.clear()
    return n
...
export async function forceSyncOmniRouteModels(args: {
  ...
  const compressionMetaFetcher = args.compressionMetaFetcher ?? defaultOmniRouteCompressionMetaFetcher
  const providersFetcher = args.providersFetcher ?? defaultOmniRouteProvidersFetcher
  const logger =
    args.logger ?? createLogger(resolved.features?.startupDebug ? "debug" : (resolved.features?.logLevel ?? "warn"))
  ...
  const auth = await resolveOmniRouteRuntimeAuth(resolved, args.readAuthJson ?? defaultReadAuthJson)
  if (!auth) {
    return {
      ok: false,
      count: 0,
      ...
      clearedMemory: 0,
      clearedDisk: false,
      error: "No OmniRoute credentials/baseURL available. Run `opencode connect omniroute` or set plugin baseURL.",
    }
  }
  ...
  try {
    ...
    if (wantAutoCombos) {
      try {
        rawAutoCombos = await autoCombosFetcher(auth.baseURL, auth.managementReadToken, 5_000, logger)
      } catch {
        /* soft-fail */
      }
    }
    ...
    if (wantCompressionMeta) {
      try {
        rawCompressionCombos = await compressionMetaFetcher(auth.baseURL, auth.managementReadToken, 10_000)
      } catch {
    ...
    if (wantDiskCache) {
      try {
        const fingerprint = diskSnapshotIdentityFingerprint(auth.baseURL, auth.apiKey, auth.managementReadToken)
        const { expiresAt: _expiresAt, ...diskEntry } = entry
        await defaultDiskSnapshotWriter(resolved.providerId, diskEntry, fingerprint)
        if (resolved.omnirouteProviderId !== resolved.providerId) {
          await defaultDiskSnapshotWriter(resolved.omnirouteProviderId, diskEntry, fingerprint)
        }
      } catch {
...
export function createOmniRouteSyncModelsTool(args: {
  ...
  return tool({
    ...
    args: {
      reason: tool.schema.string().optional().describe("Optional reason for the sync (logging only)"),
    },
  ...
  const tick = () => {
    ...
    inFlight = (async () => {
      ...
      if (result.count !== lastCount) {
        logger.info(
          `auto-sync catalog size changed ${lastCount} → ${result.count} ` + `(providerId=${resolved.providerId})`,
        )
        lastCount = result.count
      }
    })()
...
export const OmniRoutePlugin: Plugin = async (_input, options) => {
  ...
  const logger = createLogger(resolved.features?.startupDebug ? "debug" : (resolved.features?.logLevel ?? "warn"))
  ...
  return {
    ...
    "command.execute.before": async (input, output) => {
      if (input.command === "omni-sync") {
        const result = await forceSyncOmniRouteModels({ resolved, cache: sharedCache, logger })
        const textPart = output.parts.find((p) => p.type === "text")
        const summary = result.ok
          ? `[OmniRoute] Sincronização concluída: ${result.count} modelos, ${result.combos} combos obtidos de ${result.baseURL}. Snapshot persistido com sucesso.`
          : `[OmniRoute] Falha na sincronização: ${result.error ?? "erro desconhecido"}`
 ...
 */
export const defaultOmniRouteModelsFetcher: OmniRouteModelsFetcher = async (baseURL, apiKey, timeoutMs = 30_000) => {
 ...
 */
export const defaultOmniRouteCombosFetcher: OmniRouteCombosFetcher = async (baseURL, apiKey, timeoutMs = 10_000) => {
  if (!apiKey) throw new Error("@omniroute/opencode-plugin: apiKey required to fetch /api/combos")
  if (!baseURL) throw new Error("@omniroute/opencode-plugin: baseURL required to fetch /api/combos")
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`@omniroute/opencode-plugin: GET ${url} failed: ${res.status} ${res.statusText}`)
    }
    const body = (await res.json()) as unknown
    const rawList: unknown[] = Array.isArray(body)
      ? body
...
export function mapComboToModelV2(
  ...
  const contextValues = members.map((m) => m.context_length).filter((v): v is number => typeof v === "number" && v > 0)
  const outputValues = members
    .map((m) => m.max_output_tokens)
    .filter((v): v is number => typeof v === "number" && v > 0)
  const inputValues = members.map((m) => m.max_input_tokens).filter((v): v is number => typeof v === "number" && v > 0)

  const everyDeclaresInput = hasMembers && inputValues.length === members.length

  const capabilities: ModelV2["capabilities"] = {
    temperature: hasMembers && members.every((m) => (m.capabilities?.temperature ?? true) !== false),
    reasoning: hasMembers && members.every((m) => Boolean(m.capabilities?.reasoning || m.capabilities?.thinking)),
    attachment:
      hasMembers && members.every((m) => Boolean(m.capabilities?.attachment ?? m.capabilities?.vision ?? false)),
  ...
  const comboApiBlock = (() => {
    ...
    const allAnthropic = members.every((m) => resolveApiBlock(m.id, baseURL, apiFormat).id === "anthropic")
    return allAnthropic
 ...
 */
export function mapAutoComboToStaticEntry(autoCombo: OmniRouteRawAutoCombo): OmniRouteStaticModelEntry {
      ...
      if (providers && typeof providers === "object") {
        for (const [providerAlias, slot] of Object.entries(providers)) {
          ...
          const providerCanonical =
            typeof canonicalRaw === "string" && canonicalRaw.length > 0 ? canonicalRaw : providerAlias
      ...
      if (body && typeof body === "object" && !Array.isArray(body)) {
        for (const [providerAlias, slot] of Object.entries(body as Record<string, unknown>)) {
          ...
          for (const [modelId, raw] of Object.entries(slot as Record<string, unknown>)) {
            ...
            const cacheRead =
              typeof p.cached === "number" ? p.cached : typeof p.cacheRead === "number" ? p.cacheRead : undefined
  ...
  try {
    ...
    if (res.ok) {
      ...
      for (const fm of perModel) {
        ...
        const monthlyTokens = typeof fmObj.monthlyTokens === "number" ? fmObj.monthlyTokens : undefined
        const creditTokens = typeof fmObj.creditTokens === "number" ? fmObj.creditTokens : undefined
        ...
        const candidates = [`${provider}/${modelId}`, modelId, ...(displayName ? [displayName] : [])]
      ...
      _logger.debug(`free-tier/summary: ${perModel.length} models returned, ${matched} matched enrichment entries`)
    }
  } catch {
...
async function writeStartupDiagnostics(params: {
  ...
  lines.push(`models=${modelCount} combos=${comboCount} enrichment=${enrichmentSize} autoCombos=${autoComboCount}`)
  ...
  lines.push(`enrichment: ${withName.length} with name, ${withPricing.length} with pricing, ${withFree.length} free`)
  ...
  try {
    const diagDir = process.env.OPENCODE_DATA_DIR ?? path.join(os.homedir(), ".local/share/opencode")
...
export function shortProviderLabel(enrichment: OmniRouteEnrichmentEntry | undefined): string | undefined {
 ...
 * friendly name.
 */
export function applyProviderTag(model: ModelV2, enrichment: OmniRouteEnrichmentEntry | undefined): ModelV2 {
 ...
 * like `kiro`).
 */
export function buildCanonicalToAliasMap(enrichment: OmniRouteEnrichmentMap | undefined): Map<string, string> {
  ...
  for (const entry of enrichment.values()) {
    ...
    const canonical = typeof entry.providerCanonical === "string" ? entry.providerCanonical.trim() : ""
 ...
 */
export function buildAliasIndex(enrichment: OmniRouteEnrichmentMap | undefined): Map<string, OmniRouteEnrichmentEntry> {
...
export function resolveProviderTagEntry(
  ...
  if (direct) {
    ...
    const display = typeof direct.providerDisplayName === "string" ? direct.providerDisplayName.trim() : ""
...
export function applyEnrichment(model: ModelV2, enrichment: OmniRouteEnrichmentEntry | undefined): ModelV2 {
...
export function usableProviderAliasSet(
  ...
  for (const c of connections) {
    ...
    if (typeof c.provider === "string" && c.provider.length > 0) {
      ...
      usableCanonicals.add(c.provider.toLowerCase())
    }
  }
  ...
  if (enrichment) {
    ...
    for (const entry of enrichment.values()) {
      ...
      if (usableCanonicals.has(canonical) || usableCanonicals.has(canonical.toLowerCase())) {
        aliases.add(alias)
  ...
  for (const canonical of usableCanonicals) {
...
export function isUsableRawModelId(
  ...
  if (
    usable.aliases.has(prefix) ||
    usable.aliases.has(prefixLower) ||
    usable.canonicals.has(prefix) ||
    usable.canonicals.has(prefixLower)
  ) {
    return true
  }
  return false
 ...
 * verdict logic without divergence.
 */
export function passesModelAllowlist(id: string, visible?: ModelListFilter, hidden?: ModelListFilter): boolean {
 ...
 */
export function buildComboKey(combo: OmniRouteRawCombo, used: Set<string>, providerId: string): string {
...
export function createOmniRouteProviderHook(
  ...
  const logger =
    deps.logger ?? createLogger(resolved.features?.startupDebug ? "debug" : (resolved.features?.logLevel ?? "warn"))
  ...
  const compressionMetaFetcher = deps.compressionMetaFetcher ?? defaultOmniRouteCompressionMetaFetcher
  ...
  return {
    ...
    async models(_provider, ctx) {
      ...
      const authBaseURL = (auth as unknown as { baseURL?: unknown }).baseURL
      const providerBaseURL = (_provider as { options?: { baseURL?: unknown } } | undefined)?.options?.baseURL
      const baseURL =
        resolved.baseURL ??
      ...
      if (cached && cached.expiresAt > t) {
        ...
        if (wantUsableOnly) {
          try {
            ...
            logger.warn("/api/providers fetch failed; usableOnly filter disabled for this refresh", err)
          }
        }
      ...
      const usable =
        wantUsableOnly && rawConnections.length > 0 ? usableProviderAliasSet(rawConnections, rawEnrichment) : undefined
      ...
      for (const entry of rawModels) {
        ...
        if (wantProviderTag) {
          const tagEntry = resolveProviderTagEntry(entry.id, enrichEntry, aliasIndex, canonicalToAlias)
      ...
      for (let pass = 0; pass < MAX_COMBO_PASSES && pending.length > 0; pass++) {
        ...
        for (const combo of pending) {
          ...
          const lookupName = combo.name && combo.name.trim().length > 0 ? combo.name.trim() : combo.id
      ...
      if (rawAutoCombos.length > 0) {
        for (const autoCombo of rawAutoCombos) {
          ...
          if (
            usable &&
            (autoCombo.candidateCount === 0 ||
              !autoCombo.candidatePool?.some(
                (p: string) => usable.canonicals.has(p) || usable.aliases.has(p),
              ))
          )
            continue
 ...
 */
export function createOmniRouteFetchInterceptor(config: { apiKey: string; baseURL: string }): typeof fetch {
  ...
  return (async (input, init = {}) => {
    ...
    const targetsInference =
      requestUrl?.origin === baseOrigin && normalizedPath !== undefined && inferencePaths.has(normalizedPath)
    if (!targetsInference) {
...
const STATIC_MODALITY_VALUES: ReadonlySet<OmniRouteModalityKind> = new Set(["text", "audio", "image", "video", "pdf"])
...
export function buildStaticProviderEntry(
  ...
  for (const raw of rawModels) {
    ...
    // slot is.
    if (wantProviderTag) {
      const tagEntry = resolveProviderTagEntry(raw.id, enrichmentEntry, aliasIndex, canonicalToAlias)
  ...
  for (let pass = 0; pass < MAX_STATIC_COMBO_PASSES && pendingStatic.length > 0; pass++) {
    ...
    for (const combo of pendingStatic) {
      ...
      const hasMembers = memberEntries.length > 0
      const friendlyName = combo.name && combo.name.trim().length > 0 ? combo.name.trim() : combo.id
      const displayName = hasMembers && compressionSuffix ? `${friendlyName} ${compressionSuffix}` : friendlyName
      ...
      if (hasMembers) {
        ...
        entry.reasoning = memberEntries.every((m) => Boolean(m.capabilities?.reasoning || m.capabilities?.thinking))
        entry.temperature = memberEntries.every((m) => (m.capabilities?.temperature ?? true) !== false)
...
export function diskSnapshotPath(providerId: string): string {
  const dir = process.env.OPENCODE_DATA_DIR ?? Global.Path.data
 ...
 */
function diskSnapshotIdentityFingerprint(baseURL: string, apiKey: string, managementReadToken: string): string {
...
export const defaultDiskSnapshotReader: OmniRouteDiskSnapshotReader = async (providerId, identityFingerprint) => {
  try {
    const file = diskSnapshotPath(providerId)
    ...
    return {
      ...
      rawCompressionCombos: Array.isArray(parsed.rawCompressionCombos) ? parsed.rawCompressionCombos : [],
...
function debugLogDir(): string {
  return join(process.env.OPENCODE_DATA_DIR ?? join(homedir(), ".local", "share", "opencode"), "plugins")
}
...
export function createOmniRouteConfigHook(
  ...
  return async (input: Config) => {
    ...
    const cacheKey = modelsCacheKey(baseURL, `${apiKey}\0${managementReadToken}`)
    const snapshotFingerprint = diskSnapshotIdentityFingerprint(baseURL, apiKey, managementReadToken)
    const t = now()
    const cached = cache.get(cacheKey)
