...
export const readFile = Effect.fn("Bom.readFile")(function* (fs: FSUtil.Interface, filePath: string) {
  ...
  if (Codepage.isPowerBuilderFile(filePath)) {
    return { bom: false, text: Codepage.decode(buffer, filePath) }
  }
  return split(new TextDecoder("utf-8", { ignoreBOM: true }).decode(buffer))
})
...
export const syncFile = Effect.fn("Bom.syncFile")(function* (fs: FSUtil.Interface, filePath: string, bom: boolean) {
  ...
  yield* fs.writeWithDirs(filePath, Codepage.prepareWrite(current.text, filePath, bom))
  return current.text
})
