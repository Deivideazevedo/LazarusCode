...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const omnirouteID = ProviderV2.ID.make("opencode-omniroute")
    const getProvider = Effect.fn("Provider.getProvider")((providerID: ProviderV2.ID) =>
      InstanceState.use(
        state,
        (s) => s.providers[providerID] ?? (providerID === "omniroute" ? s.providers[omnirouteID] : undefined),
      ),
    )
    ...
    const getModel = Effect.fn("Provider.getModel")(function* (providerID: ProviderV2.ID, modelID: ModelV2.ID) {
      ...
      const effectiveProviderID =
        !s.providers[providerID] && providerID === "omniroute" && s.providers[omnirouteID]
          ? omnirouteID
          : providerID
      const provider = s.providers[effectiveProviderID]
      if (!provider) {
        const catalogProvider = s.catalog[effectiveProviderID]
        const suggestions = catalogProvider
          ? modelSuggestions(catalogProvider, modelID, runtimeFlags.enableExperimentalModels)
          : fuzzysort
              .go(effectiveProviderID, Object.keys({ ...s.catalog, ...s.providers }), { limit: 3, threshold: -10000 })
              .map((m) => m.target)
        return yield* new ModelNotFoundError({ providerID: effectiveProviderID, modelID, suggestions })
      }
      ...
      if (!info) {
        ...
        const suggestions = current.length
          ? current
          : modelSuggestions(s.catalog[effectiveProviderID], modelID, runtimeFlags.enableExperimentalModels)
        return yield* new ModelNotFoundError({ providerID: effectiveProviderID, modelID, suggestions })
      }
      return info
    })
...
export function parseModel(model: string) {
  ...
  const providerID = rawProviderID === "omniroute" ? "opencode-omniroute" : rawProviderID
  return {
    providerID: ProviderV2.ID.make(providerID),
